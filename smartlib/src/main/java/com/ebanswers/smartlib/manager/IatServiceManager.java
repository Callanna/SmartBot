...
/**
 * Created by Callanna on 2016/10/25.
 */

public class IatServiceManager {
    private static IatServiceManager instance;
    // 语音听写对象
    private SpeechRecognizer mIat;

    private SpeechDialog speechDialog;

    private IatServiceManager(Context context){
        mIat = SpeechRecognizer.createRecognizer(context, mInitListener);
        speechDialog = new SpeechDialog(context);
    }
    public static IatServiceManager getInstance(Context context) {
        synchronized (IatManager.class) {
            if (instance == null) {
                instance = new IatServiceManager(context);
            }
            return instance;
        }
    }
